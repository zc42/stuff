import game_15.board as brd
import game_15.game_functions as fnc
from g15_new.lessons.objective_goto_base.reward_4_base_objctv import need_exit_4_base, calc_reward_4_base
from game_15.g15_context import Ctx, circle1, circle2
from game_15_tail.check_state_hist import is_state_in_history
from game_15_tail.mask_tester import manhattan_heuristic, is_all_masks_ok, test_masks_14_ok
...
def is_terminal_step(ctx=Ctx, f=bool):
    ...
    if rsn == None and ctx.step_count >= fnc.max_step_count(ctx): rsn = "ctx.step_count >= fnc.max_step_count(ctx)"
    # ---------------------
    ctx.exit_rsn = rsn
    return rsn != None
...
def calc_reward(ctx=Ctx, f=bool):
    ...
    r1 = (6 - brd.distance_from_hole_to_goal_number(s, g, brd.hole)) * 1e-02
    r2 = (15 - brd.distance_from_origin(s, g)) * 1e-02
...
def calc_reward_4_distance_from_goal(ctx=Ctx, goal=None):
    v = brd.distance_from_v1_to_v2(ctx.state, goal, brd.hole)
    v = 0 if v < 2 else v
    v = 6 - v
    return v
